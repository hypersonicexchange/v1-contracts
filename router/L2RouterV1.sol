...
contract HypersonicRouterV1 is Ownable {
    ...
    ***************************************************************/
    ...
    function _execute_hyperswap(GenericInfo memory swap_info, bytes calldata path) internal returns (uint256 user_amount) {
        ...
        uint256 out_amount = HYPERSONIC_EXECUTOR.execute(IHYPERSONICEXECUTOR.GenericInfo({in_token: IN, out_token: OUT, in_amount: swap_info.in_amount, min_out_amount: swap_info.min_out_amount}), path);
    ...
    **************************************************************/
    ...
    // @notice Add new address to cache storage slot for usage when storage is cheaper than calldata.
    ...
    /// @notice Add new addresses in bulk to cache storage slot for usage when storage is cheaper than calldata.
    /// @param idxs Array of storage index.
    /// @param addrs Array of addresses to cache.
    function bulk_CACHE(uint256[] calldata idxs, address[] calldata addrs) external onlyOwner {    
        assembly {
